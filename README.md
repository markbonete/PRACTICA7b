...
- #include <WiFi.h> i #include <ESP8266WiFi.h>: Inclou les llibreries WiFi segons la plataforma (ESP32 o ESP8266).
- Inclou altres llibreries per manejar fonts d'àudio (AudioFileSourceICYStream.h, AudioFileSourceBuffer.h), generadors d'àudio (AudioGeneratorMP3.h, AudioGeneratorAAC.h), sortida d'àudio (AudioOutputI2S.h), i la memòria EEPROM.
...
int retryms = 0;
```
- Es defineixen les variables de configuració de la xarxa WiFi.
- Es crea un servidor web a port 80.
- Es declaren les variables globals per gestionar el flux d'àudio, el volum, títol, URL, estat, i altres flags de control.

### Capçalera HTML i cos
```cpp
static const char HEAD[] PROGMEM = R"KEWL(
<head>
<title>ESP8266 Web Radio</title>
...
```
- HEAD i BODY contenen els components HTML que es mostraran a la interfície web per controlar la ràdio.

### Funcions de manipulació de pàgines web
```cpp
void HandleIndex(WiFiClient *client)
{
 char buff[sizeof(BODY) + sizeof(title) + sizeof(status) + sizeof(url) + 3*2];
...
```
- HandleIndex(): Genera i envia la pàgina principal HTML al client.

### Funcions de gestió de l'estat i volum
```cpp
void HandleStatus(WiFiClient *client)
{
 WebHeaders(client, NULL);
...
```
- HandleStatus(): Envia l'estat actual al client.
- HandleTitle(): Envia el títol actual al client.
- HandleVolume(): Ajusta el volum basant-se en els paràmetres passats per URL.

### Funcions per canviar URL i aturar la reproducció
```cpp
void HandleChangeURL(WiFiClient *client, char *params)
{
 char *namePtr;
...
```
- HandleChangeURL(): Canvia l'URL de la ràdio segons els paràmetres passats per URL.
- HandleStop(): Atura la reproducció de l'àudio.

### Funcions de control d'àudio i configuració
```cpp
void StartNewURL()
void StopPlaying()
void LoadSettings()
...
void PumpDecoder()
```
- StartNewURL(): Inicia la reproducció de l'àudio des d'un nou URL.
- StopPlaying(): Atura la reproducció de l'àudio i allibera els recursos.
- LoadSettings(): Carrega la configuració des de l'EEPROM.
- SaveSettings(): Desa la configuració a l'EEPROM.
- PumpDecoder(): Gestiona el bucle de descompressió i reproducció de l'àudio.

### Setup i Loop
- setup(): Inicialitza el sistema, connecta a la xarxa WiFi i prepara el servidor web.
- loop(): Gestiona les peticions dels clients web, maneja la reproducció de l'àudio i controla l'estat de la ràdio.
